 ...
 *  virtual chinrest plugin for jsPsych, based on Qisheng Li 11/2019. /// https://github.com/QishengLi/virtual_chinrest
    ...
    Modified bt Clara Kuper 02/2022 to enable touch responses
...
jsPsych.plugins["virtual-chinrest"] = (function () {
    ...
    parameters: {
      ...
      adjustment_prompt: {
        ...
        default: `
          ...
          <p>Touch and drag the lower right corner of the image until it is the same size as a credit card held up to the screen.</p>
      ...
      adjustment_start_button_prompt: {
        type: jsPsych.plugins.parameterType.HTML_STRING,
        pretty_name: "Adjustment start button prompt",
        default: "Click here when you're ready to start resizing",
        description:
          " Content of the button displayed below the card instruction.",
      },
      adjustment_end_button_prompt: {
        type: jsPsych.plugins.parameterType.HTML_STRING,
      ...
      blindspot_prompt: {
        ...
        default: `
          ...
          <div style="text-align: left">
            <ol>
              <li>Put your left index just above the <b>touch screen</b>.</li>
              ...
              <li>The <span style="color: red; font-weight: bold;">red ball</span> will disappear as it moves from right to left. Tap on the screen as soon as the ball disappears.</li>
            </ol>
          </div>
          `,
      ...
      blindspot_start_prompt: {
        ...
        default: "Click here when you're ready to start.",
  ...
  plugin.trial = function (display_element, trial) {
    ...
    /* create content for sizing instruction */
    let pagesize_instruction_content = `
      <div id="page-size-instruction">
        ${trial.adjustment_prompt}
        <button id="start_resize_phase" class="jspsych-btn">
          ${trial.adjustment_start_button_prompt}
        </button>
      </div>
    `
    /* define the credit card as string */
    let message = false;
    ...
    let pagesize_content = `
      <div id="page-size">
        <div class="box" id="item"><div class="chip"></div><div class="number"><h>0000 0000 0000</h></div>
        ...
        <button id="end_resize_phase" class="jspsych-btn" style="bottom:20px; left:calc(50% - 150px); position:absolute;">
          ${trial.adjustment_end_button_prompt}
        </button>
      </div>
    `

    /* create instruction content for blindspot */
    let blindspot_instruction_content = `
      <div id="blind-spot">
        ${trial.blindspot_prompt}
        <button class="btn btn-primary" id="start_blindspot_phase"> 
    ...
    display_element.innerHTML = `<div id="content" style="width:${window.innerWidth}px; margin: 0 auto;"></div>`

    const start_time = performance.now();
    showResizePrompt();

    function showResizePrompt(){
      display_element.querySelector('#content').innerHTML = pagesize_instruction_content;
      display_element.querySelector("#start_resize_phase").addEventListener("click", startResizeLaura);
    };

    function startResizeLaura() {
      display_element.querySelector('#content').innerHTML = pagesize_content;

      if (message && !message_printed){
            display_element.innerHTML += "<p class='p1' id='invalid_card' style = 'bottom:70px; left:calc(50% - 200px); position:absolute;'>This doesn't look like a credit card yet!</p>";
       ...
       function touchendevent(){
           // define what happens when the user stops touching the screen
           dragging = false;
       }
       ...
       function touchstartevent(event){
           // define what happens when the user start touching the resize handle
            event.preventDefault();
            dragging = true;
            origin_x = event.x || event.pageX || event.touches[0].clientX;
            origin_y = event.y || event.pageY || event.touches[0].clientY;
       ...
       function resizeevent(event){
           // define what happens if the hand moves around on the screen
            if (dragging) {
                let newx = event.x || event.pageX || event.touches[0].clientX;
                let newy = event.y || event.pageY || event.touches[0].clientY;
                let dx = newx - origin_x;
                let dy = newy - origin_y;
    ...
    function checkResizeValidity(){
        ...
        if (currRatio > 1.1*aspect_ratio || currRatio < 0.9*aspect_ratio){
            // repeat if wrong
            message = true;
            startResizeLaura();
        } else {
    ...
    function showBlindSpotPrompt(){
      display_element.querySelector('#content').innerHTML = blindspot_instruction_content;
      display_element.querySelector("#start_blindspot_phase").addEventListener("click", startBlindSpotPhase);
    }
    ...
    function startBlindSpotPhase() {
      ...
      // Todo: instead of waiting of a space bar, we want to wait for a touch start event
      ...
      // Watch out: Clara custom function
      document.addEventListener('touchstart', startBall);
    }

    function startBall() {
      // Clara custom
      document.removeEventListener('touchstart', startBall);
      document.addEventListener('touchstart', recordPosition);
    ...
    function drawBall(pos = 120) {
      // pos: define where the fixation square should be (in mm).
      var mySVG = SVG("svgDiv");
      const rectX = window.innerWidth; //trial_data["px2mm"] * pos;
      const ballX = rectX * 0.6; // define where the ball is
      var ball = mySVG.circle(30).move(ballX, 50).fill("#f00");
      window.ball = ball;
      var square = mySVG.rect(30, 30).move(Math.min(rectX - 100, 950), 50); //square position
