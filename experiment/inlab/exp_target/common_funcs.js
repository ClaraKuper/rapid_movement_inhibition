...
function april_tag_drawer(tag_array, square_size, c, row_start_y, col_start_x){

    let rectangle_array = [];
    let row_pos = row_start_y;
    for (let row = 0; row < tag_array.length; row++){
        let tag_row = tag_array[row]
        let col_pos = col_start_x;
        for (let col = 0; col< tag_row.length; col++){
            let color = ['#000000', '#FFFFFF'][tag_row[col]]
            let square = new Rectangle(col_pos, row_pos, square_size, square_size, color, c);
            rectangle_array.push(square)
            col_pos += square_size
        }
        row_pos += square_size
    }
...
let init_circles = function (x_positions, x_positions_shifted, y_positions, y_positions_shifted, radius, color, c) {
            let target_circles = [];
            for (let i = 0; i < x_positions.length; i++) {
                ...
                target_circles.push(new Circle(x, x_shifted, y, y_shifted, radius, color, i, c));
            }
