...
function Serial_canvas_loop(nTrials, cFlash, cJump, initShiftFirst, rangeDistanceBetween, maxLastPresentation, nDistanceWindows,
                            minxPos, maxxPos, minyPos, maxyPos, nDots, randomPosShift) {
    let T; // the trials
    let nT; // number of trials per condition
    let cF; // flash or no flash
    let cJ; // jump or no jump
    let cI; // what to initialize first
    ...
    let shiftOnsets; // array with shift onset times
    let dist;
    let first_range;
    let first_event_time;
    let test_stimuli = []; // the array that will later hold our design structure
    let twSize = rangeDistanceBetween/nDistanceWindows;
    ...
    for (nT = 0; nT < nTrials; nT++) {
        for (cF = 0; cF < cFlash.length; cF++) {
            for (cJ = 0; cJ < cJump.length; cJ++) {
                for (cI = 0; cI < initShiftFirst.length; cI++){
                    // set the order of changes
                     for (tW = 0; tW < rangeDistanceBetween; tW = tW + twSize){
                         ...
                         // set the time window of the distance
                         // get the distance
                         dist = tW + Math.random() * twSize
                         // get the range for the first event
                         first_range = maxLastPresentation - dist;
                         // select first event
                         first_event_time = Math.random() * first_range;
                         // compute second event as first + distance
                         if (initShiftFirst[cI] === 1){
                             shiftOnsets.push(first_event_time)
                             flashOnsets.push(first_event_time + dist)
                         } else {
                             flashOnsets.push(first_event_time)
                             shiftOnsets.push(first_event_time + dist)
                         }
